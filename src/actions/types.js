export const GET_ALL_SALONS_SUCCESS = "GET_ALL_SALONS_SUCCESS";
export const GET_ALL_SALONS_ERROR = "GET_ALL_SALONS_ERROR";

export const ADD_SALON_SUCCESS = "ADD_SALON_SUCCESS";
export const ADD_SALON_FAIL = "ADD_SALON_FAIL";

export const GET_LOCATIONS_SUCCESS = "GET_LOCATIONS_SUCCESS";
export const GET_LOCATIONS_FAIL = "GET_LOCATIONS_FAIL";

export const ADD_LOCATION_SUCCESS = "ADD_LOCATION_SUCCESS";
export const ADD_LOCATION_FAIL = "ADD_LOCATIONS_FAIL";

export const GET_TYPES_SUCCESS = "GET_TYPES_SUCCESS";
export const GET_TYPES_FAIL = "GET_TYPES_FAIL";

export const ADD_TYPE_SUCCESS = "ADD_TYPE_SUCCESS";
export const ADD_TYPE_FAIL = "ADD_TYPE_FAIL";

export const GET_SALON_SUCCESS = "GET_SALON_SUCCESS";
export const GET_SALON_ERROR = "GET_SALON_ERROR";

export const LOGIN_SUCCESS = "LOGIN_SUCCESS";
export const LOGIN_ERROR = "LOGIN_ERROR";

export const LOGOUT_SUCCESS = "LOGOUT_SUCCESS";
export const LOGOUT_ERROR = "LOGOUT_ERROR";

export const GET_SUB_TYPES_SUCCESS = "GET_SUB_TYPES_SUCCESS";
export const GET_SUB_TYPES_FAIL = "GET_SUB_TYPES_FAIL";

export const ADD_SUB_TYPE_SUCCESS = "ADD_SUB_TYPE_SUCCESS";
export const ADD_SUB_TYPE_FAIL = "ADD_SUB_TYPE_FAIL";

export const SALON_LOADING_TRUE = "SALON_LOADING_TRUE";
export const SALON_LOADING_FALSE = "SALON_LOADING_FALSE";

export const PORT = "https://final-ecrew-server.herokuapp.com/api";
// export const PORT = "http://localhost:8000/api";
